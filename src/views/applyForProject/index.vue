<template>
  <div>
    <van-notice-bar wrapable :scrollable="false" mode="closeable">
      ...
      请根据下方表格录入项目名称、预约工资款金额、工资款到账时间、跟进业务员姓名、联系方式信息，提交即可
      <br>
    ...
    <van-cell title-class="cell-t" label="自到账以后三个工作日后可提取" title="预约工资款时间" :value="formData.arrivalTime" @click="showFlag = true;" is-link />
    ...
    <van-field label-width="110" v-model="formData.mobile" type="tel" label="联系方式" placeholder="请输入申请人联系方式" ></van-field>
    <div class="open-ccb-box">
      <div class="open-ccb" @click="submitForm">提交</div>
    </div>
    <van-calendar title="预约工资款到账时间" v-model="showFlag" @confirm="onConfirm" />
  </div>
</template>

<script src="./applyForProject.js"></script>

<style lang="stylus">
  @import '~@/styles/common.styl';
  @import '~@/styles/applyForProject.styl';
  .cell-t
    width: 40%
    flex: auto
</style>
